...
describe Mtwarden::BraintreePlanFetcher do
	let(:faux_plan) do
		double("Plan", 
			:id    => "faux1",
			:name  => "Starter",
		...
		it "fetches and stores plans" do
			...
			expect(Mtwarden::Plan).to receive(:create).with({
				:braintree_id => "faux1",
				:name         => "Starter",
				:price        => "9.95"
		})
	...
	it "checks and updates plans" do
		expect(Braintree::Plan).to receive(:all).and_return([faux_plan])
		expect(Mtwarden::Plan).to receive(:find_by).
		...
		expect(Mtwarden::Plan).to_not receive(:create)
		Mtwarden::BraintreePlanFetcher.store_locally
	end
end